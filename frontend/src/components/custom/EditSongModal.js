...
export default function EditSongModal({ isOpen, setIsOpen, formData, setFormData, errors, handleSubmit, handleTitle, handleFileChange }) {
  return (
        ...
        <Dialog.Panel className="w-full max-w-lg rounded p-6 shadow-red text-white bg-black">
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-white">
              ...
              <div className="flex justify-end">
                <button type="button"className="border border-red-600 px-4 py-2 mr-2"onClick={() => setIsOpen(false)}>Cancel</button>

                <button type="submit"className="border border-red-600 bg-red-600 text-white px-4 py-2">Save</button>
              </div>
            </form>
        </Dialog.Panel>
      </div>
